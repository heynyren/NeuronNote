...
/**
   ...
   * DOM không chạm tới — chữ trên đó sinh ra MỘT LẦN lúc dựng, phải đợi biết
   * thứ tiếng rồi mới dựng. Đọc cài đặt là một lượt hỏi bất đồng bộ; trên trang
   * nhẹ, bảng dựng xong trước khi câu trả lời về.
   */
  const datChu = (st) => {
    if (!self.Chu) return;
  ...
  function dungBang() {
    ...
    host.setAttribute("data-nnote-yt", "1");   // content.js nhìn dấu này để không cướp sự kiện
    ...
    nm.textContent = T("Neuron Note · Lời thoại");
    top.appendChild(nm);
    ...
    /*
     * Bôi đen trong bảng rồi lưu bằng phím tắt / menu chuột phải.
     *
  ...
  /**
   * Cửa cho content.js: "đang có đoạn lời thoại nào vừa được bôi đen không?".
   * Có hạn dùng 60 giây — quá đó thì coi như bạn đã sang việc khác, trả về
   * rỗng để content script quay lại cách bắt vùng chọn thông thường.
   */
  window.__NN_ytChon = function () {
    if (!vungVuaChon || Date.now() - vungVuaChon.luc > 60000) return null;
